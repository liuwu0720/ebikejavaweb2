...
public class test {
	...
	public static void edbProductGet() {
		...
		for (Iterator<Map.Entry<String, String>> it = apiparamsMap.entrySet()
			...
			if (e.getKey() != "appscret" && e.getKey() != "token") {
				if (e.getKey() == "shop_id" || e.getKey() == "wangwang_id"
						|| e.getKey() == "date_type") {
					try {
						param.append("&").append(e.getKey()).append("=")
								.append(Util.encodeUri(e.getValue()));
	...
	public static void edbTradeAdd() {
		...
		apiparamsMap.put("timestamp", timestamp);// 添加请求参数——时间戳
		/*
		 * shop_name,店铺名称 wangwang 旺旺号 level 用户级别，传入名称 customer_type 客户分组
		 * 可以传多个，，以,号分隔 buy_num 交易 次数 order_money 订货总额 custom_1 星座 ----value_1
		 * 星座名称 custom_2 生日类型 ----value_2生日名称 custom_3 情感状态 ----value_3 情感名称
		 * custom_4 血型 ----value_4 血型名称 custom_5 生肖 ----value_5 生肖名称
		 */
