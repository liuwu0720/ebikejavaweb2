...
<body>
	...
	<div class="maindiv">
		...
		<table id="main" class="table table-condensed" border="1"
			...
			<tr>
				...
				<th>申请日期</th>
				<td><fmt:formatDate value="${ddcHyxhSsdwclsb.sqrq }"
			...
			<tr>
				<th>驾驶人1姓名</th>
				<td>${ddcHyxhSsdwclsb.jsrxm1 }(${ddcHyxhSsdwclsb.vcJsr1State })</td>
				<th>身份证号码1</th>
			...
			<tr>
				...
				<th>驾驶人2性别</th>
				<td><c:if test="${ddcHyxhSsdwclsb.xb2==0 }">
    			 	男
    			 </c:if> <c:if test="${ddcHyxhSsdwclsb.xb2==1 }">
    			 	女
    			 </c:if></td>
			...
			<tr>
				...
				<th>办理日期</th>
				<td><fmt:formatDate value="${ddcHyxhSsdwclsb.slrq }"
						pattern="yyyy-MM-dd" /></td>
				<th>办理结果</th>
			...
			<c:if test="${ddcHyxhSsdwclsb.slyj==1 }">
				<tr>
					<th>退办原因</th>
					<td colspan="7"><c:forEach items="${selectlTbyy }" var="tb">
							<p>${tb.dmms1 }</p>
						</c:forEach></td>
				</tr>
			</c:if>
			<c:if test="${ddcHyxhSsdwclsb.slyj==0 }">
				<tr>
					<th>受理资料</th>
					<td colspan="7"><c:forEach items="${selectSlzls }" var="tb">
							<p>${tb.dmms1 }</p>
						</c:forEach></td>
				</tr>
			</c:if>
			<tr>
				<th>办结意见</th>
				<td colspan="7">${ddcHyxhSsdwclsb.slbz }</td>
			</tr>
			...
			<tr>
				<td colspan="2">
					<div class="imgdiv">
						<p>购车发票照片</p>
						<a href="${ddcHyxhSsdwclsb.vcEbikeInvoiceImgShow }"
							target="_blank"> <img
							src="${ddcHyxhSsdwclsb.vcEbikeInvoiceImgShow }" />
						</a>
					</div>
				</td>
				<td colspan="2">
				...
				<td colspan="2"><a
					href="${ddcHyxhSsdwclsb.vcQualifiedImgShow }" target="_blank">
						<div class="imgdiv">
							<p>车辆合格证</p>
				...
				<td colspan="2"><a
					href="${ddcHyxhSsdwclsb.vcEbikeInsuranceImgShow }" target="_blank">
						<div class="imgdiv">
							<p>投保凭证</p>
			...
			<table class="table table-condensed">
				...
				<c:forEach items="${approveUsers }" var="approve">
					<tr>
						...
						<td><fmt:formatDate value="${approve.approveTime }"
	...
	<div id="dgformDiv" class="easyui-dialog"
		style="width:550px;padding:10px 20px 20px 20px;" closed="true"
		...
		<form id="dgform" class="easyui-form" method="post">
			<div class="tbdiv">
				...
				<ul>
					<li><p>退办原因:</p>
					<li>
					<li><c:forEach items="${dbyyDdcSjzds }" var="bg">
							<input type="checkbox" id="sltbyyzls" name="sltbyyzls"
								value="${bg.dmz}" />${bg.dmms1 }<br />
						</c:forEach></li>
					<li><p>备注:</p></li>
		...
		<form id="dgform2" class="easyui-form" method="post">
			<div class="tbdiv">
				...
				<ul>
					<li><p>受理资料:</p>
					<li>
					<li><c:forEach items="${slzList }" var="bg">
							<input type="checkbox" id="slzList" name="slzList"
								checked="checked" value="${bg.dmz}" />${bg.dmms1 }<br />
						</c:forEach></li>
					<li><p>备注:</p></li>
					<li><textarea rows="10" cols="65" name="note"></textarea></li>
				</ul>
